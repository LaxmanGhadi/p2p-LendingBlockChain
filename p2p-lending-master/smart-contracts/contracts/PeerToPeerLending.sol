...
contract PeerToPeerLending is Destructible {
    ...
    function applyForCredit(
        ...
        users[msg.sender].activeCredit = address(cre);

        // Add the credit contract to our list with contracts.
        credits.push(address(cre));

        // Add the credit to the user's profile.
        users[msg.sender].allCredits.push(address(cre));

        // Log the credit creation event.
        emit LogCreditCreated(address(cre), msg.sender, block.timestamp);
