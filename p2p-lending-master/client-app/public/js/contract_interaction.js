...
$(document).ready(function () {
  let peerToPeerLendingContract = "0xA73FD73AAf274416c93f779C93e338de32042c5d";
  ...
  let peerToPeerLendingContractInstance = new ethereum.Contract(
    peerToPeerLendingContractABI
  ).at(peerToPeerLendingContract);
  ...
  peerToPeerLendingContractInstance.getCredits(function (err, result) {
    ...
    result.forEach(function (index) {
      ...
      let creditContract = new ethereum.Contract(creditContractABI).at(
        creditAddress
      );
      ...
      creditContract.getCreditInfo(function (err, info) {
        ...
        $("#activeCredits").append(`<div class="col-sm-6 mt-5">
                                <div class="card">
                                    <div class="card-header ${
                                      creditContractInfo.active == true
                                        ? "alert-success"
                                        : "alert-secondary"
                                    }">
                                        <h5 class="card-title">${
                                    ...
                                    <div class="card-body">
                                        ...
                                        <p class="card-text">
                                            ...
                                            }" >${creditContractInfo.borrower}</a>
                                        </p>
                                        <p class="card-text">
                                            <span class="credit-description-property">Requested:</span> ${web3.fromWei(
                                              creditContractInfo.requestedAmount,
                                              "ether"
                                            )} ETH
                                        </p>
                                        <p class="card-text">
                                            <span class="credit-description-property">Funded:</span> ${web3.fromWei(
                                        ...
                                        <p class="card-text">
                                            <span class="credit-description-property">Interest:</span> ${web3.fromWei(
                                    ...
                                    <div class="card-footer">
                                        ...
                                        <div class="mb-3">
                                            <button type="button" class="btn btn-info" ${
                                              creditContractInfo.state !== 2
                                                ? 'style="display:none;"'
                                                : ""
                                            } name="requestInterest" data-contract-address="${creditContractInfo.address}">Get interest</button>
                                            <button type="button" class="btn btn-warning" name="revokeVote" data-contract-address="${
                                              creditContractInfo.address
                                            }">Revoke vote</button>
                                            <button type="button" class="btn btn-warning" name="refund" data-contract-address="${
                                              creditContractInfo.address
                                            }">Refund</button>
                                            <button type="button" class="btn btn-danger" name="fraudVote" data-contract-address="${
                                              creditContractInfo.address
                                            }">Fraud</button>
                                        </div>
                                    </div>
  ...
  var filter = ethereum.filter("latest");
  ...
  $("body").on("click", 'button[name="invest"]', function (e) {
    ...
    let selectedCreditContract = new ethereum.Contract(creditContractABI).at(
      address
    );
  ...
  if (top.location.pathname === "/lend") {
    ...
    peerToPeerLendingContractInstance.users(
      web3.eth.coinbase,
      function (err, result) {
        if (err) {
          console.log(err);
    ...
    peerToPeerLendingContractInstance.getUserCredits(function (err, result) {
      ...
      result.forEach(function (index) {
        ...
        creditContract.getCreditInfo(function (err, info) {
          ...
          $("#myCredits").append(`<div class="col-sm-6 mt-5">
                                <div class="card">
                                    <div class="card-header ${
                                      creditContractInfo.active == true
                                        ? "alert-success"
                                    ...
                                    <div class="card-footer" ${
                                      creditContractInfo.state !== 1
                                        ...
                                        <div class="input-group mb-3 investment-card">
                                          <input type="number" class="form-control" name="amount" placeholder="Amount" aria-label="Amount" aria-describedby="basic-addon2">
                                          <div class="input-group-append">
                                            <button class="btn btn-outline-success" type="button" name="repayInstallment" data-contract-address="${
                                              creditContractInfo.address
                                            }">Repay installment</button>
                                          </div>
                                        </div>
                                        <div class="mb-3">
                                            <button type="button" class="btn btn-info" name="withdraw" data-contract-address="${
                                              creditContractInfo.address
                                            }">Withdraw</button>
                                        </div>
                                    </div>
    ...
    peerToPeerLendingContractInstance.users(address, function (err, result) {
      ...
      $(
        "#checkContent"
                            ...
                            <p>Credited: ${
                              user.credited == true ? "Yes" : "No"
                            } </p>
                            <p>Last credit: ${user.lastCredit} </p>
                            <p>Fraud status: ${
                              user.fraudStatus == true ? "Fraudlent" : "Good"
                            } </p>
                        </div>`);
    });
  }
